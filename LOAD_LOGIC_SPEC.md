...
### ① ペース由来負荷 (相対強度負荷)
距離または時間とペースが入力されている場合に適用されます。ユーザー本人の走力（閾値ペース）を基準として計算されます。
- **計算式**: `練習時間(分) × (基準となる閾値ペース / 実際のエース)^3 × 3`
- **基準となる閾値ペース**: 自己ベストから算出された VDOT に基づく閾値（T）ペースを使用します。自己ベストがない場合は 5:00/km (300秒) を暫定値とします。
- **特徴**: 3乗の重み付けをすることで、ジョグ(低強度)の負荷が過大に出るのを防ぎ、インターバル(高強度)の重みを適切に評価します。
