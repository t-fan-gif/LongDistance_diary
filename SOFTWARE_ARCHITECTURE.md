...
## 設計上の未決事項（後で確定）
- ルーティング方式（`go_router` 等）
- ID方式（整数/UUID）
- エクスポート/インポートのフォーマットと範囲（JSON/CSV、暗号化の要否）

## 完了条件チェックリスト
- [ ] レイヤ（UI/アプリケーション/ドメイン/永続化）の責務が説明されている
- [ ] モジュール分割（`lib/core`/`lib/features`）が要件と対応している
- [ ] 集計パイプライン（代表負荷→日負荷→能力推定→濃淡）が説明されている
- [ ] 未決事項が列挙され、決める場所（方式or詳細or運用）が分かる
