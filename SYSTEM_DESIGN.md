...
## エクスポート・インポート（データ同期）の方針
- 対象: Session（実績）/ Plan（予定）/ PersonalBest（PB）/ メニュープリセット / 日毎のメモ
- 方式: JSON形式によるインポート/エクスポートを提供し、インポート時は既存データを維持したまま差分を統合（マージ）する方式を採用する（ID重複時のみ上書き）。
- 詳細は `SOFTWARE_DETAILED_DESIGN.md` を参照。
...
## 方式の根拠（要件とのトレース）
- offline-first / 端末内保存:
  - `SYSTEM_REQUIREMENTS.md` の「運用前提（offline-first）」および「データ取り扱い（外部送信の限定）」に対応。
- アカウント/同期をMVPで実施しない:
  - `SYSTEM_REQUIREMENTS.md` の「運用前提（アカウント/同期は必須にしない）」に対応。
- JSONエクスポート:
  - `SYSTEM_REQUIREMENTS.md` の「データ保全（退避手段を用意できる設計）」に対応。
- 自動の外部送信を行わない:
  - `SYSTEM_REQUIREMENTS.md` の「データ取り扱い（外部送信は明示的操作に限定）」に対応。

## 完了条件チェックリスト
- [x] システム境界（対象/対象外）と外部I/Fが明記されている
- [x] 方式（offline-first、端末内保存）の根拠が要件とトレースできる
- [x] 対象OSバージョン（Android/iOS方針）が明記されている
- [x] 採用技術（Flutter/SQLite+drift/riverpod）が「何のためか」説明されている
- [x] 運用・配布（データ保全/バックアップ導線）の方針がある
- [x] エクスポート（形式/範囲）が定義されている
- [x] 主要な未決事項/リスクが列挙されている
